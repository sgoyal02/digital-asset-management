...
const StatusBadge = ({ status }: { status: Asset['status'] }) => {
  ...
  UNDER_REVIEW: "bg-primary-200/10 text-primary-200 border border-primary-200/30",
  ...
  return (
    <span className={`inline-block px-2 py-1.5 rounded-full text-xs font-medium ${styles}`}>
      {status.replace('_', ' ')}
    </span>
  );
};
